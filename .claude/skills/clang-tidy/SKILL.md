---
name: clang-tidy
description: Run or diagnose clang-tidy for first-party C++ work associated with Parallel. Use for requested C++ static analysis or suppression fixes; this tool does not lint Python.
---
...
`.lintrunner.toml` also defines `CLANGTIDY`. With a compilation database for
the intended backend, `lintrunner --take CLANGTIDY` uses the adapter's file
scope. OpenMP and TBB sources are excluded from the default-backend lint
pass; validate those through a build configured for the affected backend.
