#!/usr/bin/env python3
"""Parallel CMake Build Configuration Script.
...
(PARALLEL_ENABLE_*/PARALLEL_* in CMakeLists.txt) — there is no multi-module
Library/* tree or --project.* scoping here, just one flat set of flags.

Usage:
    python Scripts/setup.py config.build.test
    python Scripts/setup.py config.build.test.coverage
    python Scripts/setup.py config.build.test.parallel.tbb
...
class ErrorLogger:
    ...
    def __init__(self, log_dir: str = "logs"):
        ...
        self.log_file = (
            self.log_dir / f"parallel_build_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
        self.errors = []
...
class ParallelFlags:
    """Maps setup.py dotted tokens to Parallel's PARALLEL_* CMake cache variables.

    Scoped 1:1 to the options CMakeLists.txt actually defines — unlike XSigma's
    XSigmaFlags there is no multi-module fan-out (Parallel is a single flat
    ...
    def __initialize_flags(self):
        ...
        self.__description = [
            ...
            "build the Parallel test suite (BUILD_TESTING; default ON)",
            "build Parallel example programs",
            "enable GoogleTest for Parallel (default ON; token disables it)",
            "enable Google Benchmark for Parallel (default ON; token disables it)",
            ...
            "SMP backend: --parallel.std, --parallel.openmp, or --parallel.tbb",
    ...
    def __build_cmake_flag(self):
        ...
        self.__name = {
            ...
            "test": "PARALLEL_ENABLE_TESTING",
            "examples": "PARALLEL_ENABLE_EXAMPLES",
            "gtest": "PARALLEL_ENABLE_GTEST",
            "benchmark": "PARALLEL_ENABLE_BENCHMARK",
            "icecc": "PARALLEL_ENABLE_ICECC",
            "cache": "PARALLEL_ENABLE_CACHE",
            "cache_type": "PARALLEL_CACHE_BACKEND",
            "clangtidy": "PARALLEL_ENABLE_CLANGTIDY",
            "fix": "PARALLEL_ENABLE_FIX",
            "iwyu": "PARALLEL_ENABLE_IWYU",
            "sanitizer": "PARALLEL_ENABLE_SANITIZER",
            "sanitizer_enum": "PARALLEL_SANITIZER_TYPE",
            "spell": "PARALLEL_ENABLE_SPELL",
            "valgrind": "PARALLEL_ENABLE_VALGRIND",
            "linker": "PARALLEL_LINKER_CHOICE",
            "coverage": "PARALLEL_ENABLE_COVERAGE",
            "cxxstd": "PARALLEL_CXX_STANDARD",
            "lto": "PARALLEL_LTO_MODE",
        ...
        self.__value.update(
            {
                ...
                "test": self.ON,  # PARALLEL_ENABLE_TESTING default ON
                "gtest": self.ON,  # PARALLEL_ENABLE_GTEST default ON
                "benchmark": self.ON,  # PARALLEL_ENABLE_BENCHMARK default ON
                "cache": self.ON,  # PARALLEL_ENABLE_CACHE default ON
                "cache_type": "none",
                "linker": "default",
    ...
    def __validate_flags(self):
        ...
        if parallel_backend not in ["std", "openmp", "tbb"]:
            print_status(f"Invalid SMP backend '{parallel_backend}'. Valid options: std, openmp, tbb", "ERROR")
            sys.exit(1)
...
class ParallelConfiguration:
    def __init__(self, args_list):
        ...
        self.__parallel_flags = ParallelFlags(args_list)
    ...
    def __set_ninja_flags(self):
        ...
        self.__value["build_folder"] = f"build_ninja{self.__parallel_flags.builder_suffix}"
    ...
    def __set_xcode_flags(self):
        if self.__value["system"] == "Darwin" and check_xcode_availability():
            ...
            self.__value["build_folder"] = f"build_xcode{self.__parallel_flags.builder_suffix}"
    ...
    def __set_visual_studio(self, arg):
        ...
        self.__value["build_folder"] = f"{base_build_folder}{self.__parallel_flags.builder_suffix}"
    ...
    def config(self, source_path, build_path):
        ...
        try:
            cmake_flags = []
            self.__value["build_enum"] = self.__parallel_flags.create_cmake_flags(
    ...
    def cppcheck(self, source_path, build_path):
        if self.__value["build"] != "build" or not self.__parallel_flags.is_cppcheck():
            return 0
    ...
    def test(self, source_path, build_path):
        if self.__value["test"] != "test":
            return 0
        if self.__parallel_flags.is_valgrind():
        ...
        return test_helper.run_ctest(
            ...
            self.__shell_flag(),
            sanitizer_type=self.__parallel_flags.get_sanitizer_type(),
            source_path=source_path,
        )

    def coverage(self, source_path, build_path):
        if self.__value["build"] != "build" or not self.__parallel_flags.is_coverage():
            return 0
    ...
    for arg in args:
        if arg.startswith("--sanitizer."):
            ...
            if backend_value in valid_backends:
                ...
                print_status(f"Invalid SMP backend: {backend_value}. Valid options: {', '.join(valid_backends)}", "ERROR")
                sys.exit(1)
        elif arg.startswith("--linker."):
...
def main():
    if len(sys.argv) == 2 and sys.argv[1] == "--help":
        ...
        print("  5. Build against the TBB or OpenMP SMP backend:")
        print("     setup.py config.build.test --parallel.tbb")
        print("     setup.py config.build.test --parallel.openmp")
        print("\nBuild commands:")
    ...
    try:
        ...
        compilation_calc = ParallelConfiguration(arg_list)
